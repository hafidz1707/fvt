...
freq = 1000000;
weight = FindWeight(weight,N,b,TravelTime,TravelTimesTable_S2,TravelTimesTable_R2,freq,size(TravelTime,1));
...
title('Error RMS vs iterasi ke-n SIRT')
xlabel('iterasi ke-n')
ylabel('Error Velocity RMS (%)')

%% Plot Error V relatif
figure
plot(mat_k_time, mat_error_rel_ray, mat_k_time, mat_error_rel_fresnel)
title('Error Velocity Relatif vs iterasi ke-n SIRT & MSIRT')
xlabel('iterasi ke-n')
ylabel('Error Velocity Relatif (%)')
legend('Raypath','Fresnel Volume');
...
%% Gif
filename = 'CKB5mm_240_SIRT.gif';
for n = 0:1:50
    if n == 0
        Title = strcat('Model SIRT Iterasi ke - ',num2str(n),' Error : ',num2str(mat_error(n+1)),' %');
